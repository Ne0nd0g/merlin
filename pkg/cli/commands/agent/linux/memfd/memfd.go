...
func NewCommand() *Command {
	...
	cmd.os = os.LINUX
	description := "Execute a program from memory"
	// Style guide for usage https://developers.google.com/style/code-syntax
	usage := "memfd filePath [args]"
	example := "Merlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]» memfd /tmp/hello.py\n" +
	...
	notes := "Load a Linux executable file into memory (RAM) as an anonymous file using the memfd_create API " +
		"call, executes it, and returns the results. The file is created with an empty string as its name. Less the " +
		"fact that RAM is a file on Linux, the executable is not written to disk. View the Detecting Linux " +
		"memfd_create() Fileless Malware with Command Line Forensics for detection guidance.\n" +
		"\tReferences:\n" +
...
func (c *Command) Do(m menu.Menu, id uuid.UUID, arguments string) (response commands.Response) {
	...
	// TODO this API call will need to take the file bytes when the CLI is separated from the server
	msg := agentAPI.MEMFD(id, args)
