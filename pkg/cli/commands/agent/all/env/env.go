...
func (c *Command) Get(id uuid.UUID, arguments string) (response commands.Response) {
	...
	if len(args) < 3 {
		response.Message = &messages.UserMessage{
			Level:   messages.Info,
			Message: fmt.Sprintf("'%s get' command requires at least one argument\n%s", c, c.help.Usage()),
			Time:    time.Now().UTC(),
		}
		return
	}
	...
	if len(args) > 2 {
		...
		case "help", "-h", "--help", "?", "/?":
			response.Message = &messages.UserMessage{
				Level:   messages.Info,
				Message: fmt.Sprintf("'%s get' command help\n\ndescription :=\n\t%s\nUsage:\n\t%s\nExample:\n\t%s\nNotes:\n\t%s", c, h.Description, h.Usage, h.Example, h.Notes),
				Time:    time.Now().UTC(),
			}
			return
		}
	}
	msg := agentAPI.ENV(id, args)
...
func (c *Command) Set(id uuid.UUID, arguments string) (response commands.Response) {
	...
	if len(args) > 2 {
		...
		case "help", "-h", "--help", "?", "/?":
			response.Message = &messages.UserMessage{
				Level:   messages.Info,
				Message: fmt.Sprintf("'%s set' command help\n\ndescription :=\n\t%s\nUsage:\n\t%s\nExample:\n\t%s\nNotes:\n\t%s", c, h.Description, h.Usage, h.Example, h.Notes),
				Time:    time.Now().UTC(),
			}
			return
		}
	...
	if len(args) < 4 {
		response.Message = &messages.UserMessage{
			Level:   messages.Info,
			Message: fmt.Sprintf("'%s set' command requires two arguments\n%s", c, h.Usage),
			Time:    time.Now().UTC(),
		}
		return
	}
...
func (c *Command) Show(id uuid.UUID, arguments string) (response commands.Response) {
	...
	if len(args) > 2 {
		...
		case "help", "-h", "--help", "?", "/?":
			response.Message = &messages.UserMessage{
				Level:   messages.Info,
				Message: fmt.Sprintf("'%s show' command help\n\ndescription :=\n\t%s\nUsage:\n\t%s\nExample:\n\t%s\nNotes:\n\t%s", c, h.Description, h.Usage, h.Example, h.Notes),
				Time:    time.Now().UTC(),
			}
			return
		}
...
func (c *Command) Unset(id uuid.UUID, arguments string) (response commands.Response) {
	...
	if len(args) < 3 {
		response.Message = &messages.UserMessage{
			Level:   messages.Info,
			Message: fmt.Sprintf("'%s set' command requires one argument\n%s", c, h.Usage),
			Time:    time.Now().UTC(),
		}
		return
	}
	...
	case "help", "-h", "--help", "?", "/?":
		response.Message = &messages.UserMessage{
			Level:   messages.Info,
			Message: fmt.Sprintf("'%s unset' command help\n\ndescription :=\n\t%s\nUsage:\n\t%s\nExample:\n\t%s\nNotes:\n\t%s", c, h.Description, h.Usage, h.Example, h.Notes),
			Time:    time.Now().UTC(),
		}
		return
	}
