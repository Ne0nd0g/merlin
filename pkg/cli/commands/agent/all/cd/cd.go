...
func NewCommand() *Command {
	...
	cmd.os = os.ALL
	description := "Change the Agent's current working directory to provided file system path"
	usage := "cd directory"
	example := "Merlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]» cd /usr/bin\n" +
	...
	example += "Merlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]» cd /usr/bin\n" +
		...
		"\tChanged working directory to /usr/bin"
	notes := "Relative paths can be used (e.g.,. ./../ or downloads\\\\Merlin). " +
...
func (c *Command) Do(m menu.Menu, id uuid.UUID, arguments string) (response commands.Response) {
	...
	response.Message = rpc.CD(id, args[1:])
	return
}
