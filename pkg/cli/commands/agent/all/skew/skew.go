...
func NewCommand() *Command {
	...
	cmd.os = os.ALL
	description := "Change the jitter, or skew, of the Agent's beaconing interval."
	// Style guide for usage https://developers.google.com/style/code-syntax
	usage := "skew amount"
	example := "Merlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]» skew 5\n" +
		"\tMerlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]»\n" +
		"\t[-]Created job lyYQdxckTY for agent c1090dbc-f2f7-4d90-a241-86e0c0217786"
	notes := "The skew number is used to calculate a random number between 0 and the skew number. " +
		"The random number is used as milliseconds and added to the beaconing interval to create a random beaconing interval. " +
		"The skew number is used to help avoid beaconing patterns that may be detected by network defenders."
...
func (c *Command) Do(m menu.Menu, id uuid.UUID, arguments string) (response commands.Response) {
	...
	if err != nil {
		response.Message = &messages.UserMessage{
			Level:   messages.Warn,
			Message: fmt.Sprintf("there was an error converting '%s' to an integer: %s\n%s", args[1], err, c.help.Usage()),
			Time:    time.Now().UTC(),
		}
		return
	}
	msg := agentAPI.Skew(id, args)
