...
func NewCommand() *Command {
	...
	usage := "pwd"
	example := "Merlin[agent][c1090dbc-f2f7-4d90-a241-86e0c0217786]» pwd\n" +
	...
	notes := "This command use native Go code to get and return the current working directory."
...
func (c *Command) Do(m menu.Menu, id uuid.UUID, arguments string) (response commands.Response) {
	if id == uuid.Nil {
		response.Message = message.NewErrorMessage(fmt.Errorf("UUID can't be nil\n%s", c.help.Usage()))
		return
	}
	// Parse the arguments
