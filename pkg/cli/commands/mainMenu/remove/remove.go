...
func NewCommand() *Command {
	...
	cmd.os = os.LOCAL
	description := "Remove or delete an agent from the server so that it will not show up in the list of available agents."
	usage := "remove agentID"
	example := "Merlin» sessions\n\n" +
		"\t+--------------------------------------+-------------+------+--------+-----------------+--------+\n" +
		...
		"\t+------------+----------+------+------+-----------+--------+\n" +
		"\t+------------+----------+------+------+-----------+--------+"
	notes := "Use tab completion to cycle through available agents. Useful when an Agent died unexpectedly and is still listed."
...
func (c *Command) Do(m menu.Menu, id uuid.UUID, arguments string) (response commands.Response) {
	...
	if err != nil {
		response.Message = message.NewErrorMessage(fmt.Errorf("there was an error converting '%s' to a UUID: %s\n%s", args[1], err, c.help.Usage()))
		return
	}
	response.Message = rpc.Remove(id)
	return
}
